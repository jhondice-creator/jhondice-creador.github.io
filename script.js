...
const GS = {
  ...
  playerProgram:   '',
  timeLimitSec:    30,
  maxLives:        3,   // configurable desde panel docente
...
/* ════════════════════════════════════════════════════════════
   STORAGE
════════════════════════════════════════════════════════════ */
const DEFAULT_TEACHER_CODE_HASH = '03ac674216f3e15c761ee1a5e255f067953623c8b388b4459e13f978d7c846f4';
...
const Timer = {
  ...
  /* Dibuja arco y número — siempre busca el DOM fresco */
  _draw(){
    const arc  = document.getElementById('timerArc');
    const disp = document.getElementById('timerDisplay');
    const r    = GS.timerRemaining;
    const pct  = Timer._total > 0 ? r / Timer._total : 1;
    const C    = 213.6;
    if(arc){
  ...
  /* Inicia cuenta regresiva. secs=0 → sin límite */
  start(secs, onEnd){
    Timer.stop();
    ...
    Timer._draw();   // dibujo inicial inmediato
    ...
    Timer._id = setInterval(()=>{
      ...
      if(GS.timerRemaining <= 0){
        Timer.stop();
        if(Timer._onEnd) Timer._onEnd();
      }
    }, 1000);
  },

  stop(){
    if(Timer._id){ clearInterval(Timer._id); Timer._id = null; }
  }
};
...
const App = {
  ...
  goTo(id){
    ...
    if(id==='screen-instructions') App._updateInstructionsLives();
    if(id==='screen-teacher-login') App.resetTeacherLogin();
  },

  /* ── Verificación del código docente ── */
  resetTeacherLogin(){
  ...
  async verifyTeacherCode(){
    ...
    try {
      const storedHash = Store.get('teacherCodeHash') || DEFAULT_TEACHER_CODE_HASH;
      const hashHex = await hashCode(code);
      if(hashHex === storedHash){
        if(error) error.style.display = 'none';
        App.goTo('screen-teacher');
      } else {
        if(error) error.style.display = 'block';
  ...
  /* Actualiza texto de vidas en instrucciones según config docente */
  _updateInstructionsLives(){
    const t=document.getElementById('instrLivesTitle');
    const c=document.getElementById('instrLivesCount');
    const n=GS.maxLives;
    if(t) t.textContent=`${n} Vida${n!==1?'s':''}`;
    if(c) c.textContent=`${n} vida${n!==1?'s':''}`;
  },
  ...
  startGame(){
    ...
    if(nameEl && nameEl.value.trim()){
      ...
      GS.timeLimitSec  = timeEl ? parseInt(timeEl.value, 10) : 30;
    } else if(!GS.playerName){
      App.goTo('screen-player'); return;
    }
    // Siempre leer el tiempo del select si está disponible
    if(timeEl) GS.timeLimitSec = parseInt(timeEl.value, 10);
    ...
    GS.lives=GS.maxLives; GS.finished=false;
    ...
    App._setupGameUI();
    // Esperar a que la pantalla termine la transición CSS antes de cargar pregunta
    setTimeout(()=> App._loadQuestion(), 100);
  },
  ...
  _updateLivesHUD(){
    ...
    for(let i=0;i<GS.maxLives;i++)
      html+=`<span class="life-heart ${i<GS.lives?'alive':'dead'}">❤</span>`;
    el.innerHTML=html;
    // Flash al perder vida
    if(GS.lives < GS.maxLives){
      el.classList.remove('lose-life');
      void el.offsetWidth; // reflow para reiniciar animación
      el.classList.add('lose-life');
    }
  },
  ...
  _loadQuestion(){
    ...
    // Timer — delay 520ms para que la transición CSS de la pantalla termine (450ms)
    // antes de intentar manipular el SVG del arco
    Timer.stop();  // cancelar cualquier timer anterior
    const arc0 = document.getElementById('timerArc');
    const disp0= document.getElementById('timerDisplay');
    if(arc0){ arc0.style.strokeDashoffset=0; arc0.className='timer-arc'; }
    if(disp0) disp0.textContent = GS.timeLimitSec > 0 ? GS.timeLimitSec : '∞';
    setTimeout(()=> Timer.start(GS.timeLimitSec, ()=>App._timeUp()), 520);
  },
  ...
  _showFeedback(isOk, pts, explanation, isSavepoint){
    ...
    // Nota parcial + vidas (usar maxLives para evitar repeat negativo)
    const partialGrade = gradeFromCorrect(GS.correctCount, 15);
    const livesAlive = Math.max(0, GS.lives);
    const livesDead  = Math.max(0, GS.maxLives - livesAlive);
    const livesStr   = '❤'.repeat(livesAlive) + '🖤'.repeat(livesDead);
  ...
  initTeacherPanel(){
    ...
    const lv=document.getElementById('tMaxLives'); if(lv&&cfg.maxLives) lv.value=cfg.maxLives;

    const currentInput = document.getElementById('teacherCurrentCode'); if(currentInput) currentInput.value = '';
    const newInput = document.getElementById('teacherNewCode'); if(newInput) newInput.value = '';
    const confirmInput = document.getElementById('teacherNewCodeConfirm'); if(confirmInput) confirmInput.value = '';
  },
  ...
  async saveTeacherAccessCode(){
    const current = document.getElementById('teacherCurrentCode')?.value;
    const next = document.getElementById('teacherNewCode')?.value;
    const confirmNext = document.getElementById('teacherNewCodeConfirm')?.value;

    if(!current || current.length !== 4 || !/^\d+$/.test(current) ||
       !next || next.length !== 4 || !/^\d+$/.test(next) ||
       next !== confirmNext){
      alert('Por favor ingresa el código actual y un nuevo código de 4 dígitos iguales en ambos campos.');
      return;
    }

    try {
      const storedHash = Store.get('teacherCodeHash') || DEFAULT_TEACHER_CODE_HASH;
      const currentHash = await hashCode(current);
      if(currentHash !== storedHash){
        alert('El código actual no coincide. Intenta nuevamente.');
        return;
      }

      const newHash = await hashCode(next);
      Store.set('teacherCodeHash', newHash);
      document.getElementById('teacherCurrentCode').value = '';
      document.getElementById('teacherNewCode').value = '';
      document.getElementById('teacherNewCodeConfirm').value = '';
      alert('✅ Código de acceso actualizado con éxito.');
    } catch(err){
      console.error('Error al guardar nuevo código:', err);
      alert('Ocurrió un error al cambiar el código. Intenta de nuevo.');
    }
  },
  ...
  saveTeacherConfig(){
    ...
    const lv=document.getElementById('tMaxLives')?.value;
    const ml=parseInt(lv||'3',10);
    GS.maxLives = ml; // aplicar inmediatamente
    Store.set('config',{contestName:cn||'¿Quién Quiere Ser Programador Experto?',defaultTime:dt||'30',maxLives:ml});
    alert('✅ Configuración guardada. Vidas por partida: '+ml);
  },
  ...
  exportRanking(){
    ...
    const rows=list.map(r=>[
      `"${r.name}"`,`"${r.program||''}"`,
      r.score, r.correct, `${r.pct}%`,
      (r.grade||0).toFixed(1),
      `"${r.savepoint||'—'}"`,
      `"${r.date}"`
    ].join(','));
    const csv='\uFEFF'+'Nombre,Programa,Puntaje,Correctas,Porcentaje,Nota,Seguro,Fecha\n'+rows.join('\n');
    const url=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8;'}));
    const a=document.createElement('a');
    a.href=url;
    a.download=`ranking_QPE_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Cargar configuración guardada por el docente
  const savedCfg=Store.get('config');
  if(savedCfg){
    if(savedCfg.maxLives) GS.maxLives=parseInt(savedCfg.maxLives,10);
  }
  ...
  document.addEventListener('keydown',e=>{
    ...
    // Enter en el login del panel docente
    if(e.key==='Enter'&&document.getElementById('screen-teacher-login')?.classList.contains('active')) App.verifyTeacherCode();
  });
});
